//Defining arrays for particular
let completeTasksarr = [];
let incompleteTasks = [];

//Keeping the count of the total tasks.
let totalTasks = 0;
...
if (addButton) {
  ...
  addButton.addEventListener("click", function addTask() {
    ...
    iconComplete.innerHTML =
      '<i class="fa fa-circle-thin" aria-hidden="true"></i>&nbsp&nbsp';
    ...
    function completeTask(element) {
      if (element.target.getAttribute("checked") === "0") {
        element.target.children[0].innerHTML =
...
doAllComplete.addEventListener("click", function doAllComplete() {
  for (let elem of incompleteTasks) {
    elem.children[0].innerHTML =
      '<i class="fa fa-check-circle-o" aria-hidden="true"></i>&nbsp&nbsp';
...
//To display all the elements of the todo list
